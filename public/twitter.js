document.addEventListener('DOMContentLoaded', () => {
  ...
  function isValid(url) {
    return /^(https?:\/\/)?(www\.)?(twitter\.com|x\.com)\/.+\/status\/.+/i.test(url.trim());
  }
  ...
  fetchBtn.addEventListener('click', async () => {
    ...
    if (!url) return showErr('Paste a Twitter/X video link first.');
    if (!isValid(url)) return showErr('Invalid Twitter/X URL. Make sure it contains /status/.');
    ...
    try {
      const r = await fetch(`/api/twitter/info?url=${encodeURIComponent(url)}`);
      let d;
      ...
      rTitle.textContent = d.title || 'Twitter Video';
  ...
  async function startJob(format) {
    ...
    try {
      const r = await fetch('/api/twitter/download', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: urlInput.value.trim(), title: info.title, format }) });
      let d;
      try { d = await r.json(); } catch { throw new Error('Our server is currently handling other downloads. Please try again in a moment.'); }
      if (!r.ok) throw new Error(d.error);
      jobId = d.jobId;
  ...
  function startPoll() {
    ...
    poll = setInterval(async () => {
      try {
        const r = await fetch(`/api/twitter/status/${jobId}`);
        ...
        else if (d.status === 'completed') { stopPoll(); showModal('success'); saveBtn.href = `/api/twitter/retrieve/${jobId}`; jobId = null; }
  ...
  async function cancelJob() {
    ...
    try { await fetch(`/api/twitter/cancel/${id}`, { method: 'POST' }); } catch {}
  }
  ...
  const legalTexts = {
    tos: `<h4>1. Acceptance of Terms</h4><p>By using the TW-Download website ("Service"), you agree to abide by these Terms of Service.</p><h4>2. Permitted Use</h4><p>This Service is intended solely for personal, non-commercial, and educational purposes. You represent that you have the legal right or permission from the copyright owner to download any content using this tool.</p><h4>3. Intellectual Property Rights</h4><p>We respect the intellectual property of others. You must not download copyrighted material unless you own it or have explicit written permission.</p><h4>4. Disclaimer of Warranties</h4><p>The Service is provided "as is" and "as available". We make no warranties regarding the reliability, uptime, speed, or accuracy of the service.</p><h4>5. Limitation of Liability</h4><p>In no event shall the operators of TW-Download or PandyShare be liable for any damages arising out of the use of this website.</p>`,
    dmca: `<h4>DMCA Takedown & Abuse Policy</h4><p>TW-Download operates as a transient download utility. Files are generated on-the-fly and deleted immediately after download.</p><h4>Important: We do not host content</h4><p><strong>There is no database or permanent repository of files</strong> on this server.</p><h4>Reporting Abuse</h4><p>Contact us:</p><ul><li>Telegram: <a href="https://t.me/pandyshare" target="_blank" style="color:var(--cyan);">t.me/pandyshare</a></li></ul><p>We will review your inquiry within 48 hours.</p>`
  };
  ...
  window.addEventListener('beforeunload', () => { if (jobId) navigator.sendBeacon(`/api/twitter/cancel/${jobId}`); });
