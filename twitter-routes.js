...
function twRateLimiter(req, res, next) {
  const ip = req.ip;
  const now = Date.now();
...
// GET /info
router.get('/info', twRateLimiter, (req, res) => {
  const url = req.query.url;
  if (!url) return res.status(400).json({ error: 'Twitter/X URL is required' });
  ...
  ytdlp.on('close', code => {
    if (code !== 0 || !stdout.trim()) {
      const isPrivate = /login|private|permission|403|Forbidden|protected/i.test(stderr);
      const msg = isPrivate ? 'This video is private. Only public Twitter/X videos can be downloaded.' : 'Failed to retrieve video info. Please check the URL and try again.';
...
// POST /download
router.post('/download', twRateLimiter, (req, res) => {
  ...
  const safeTitle = (title || 'twitter').replace(/[^a-zA-Z0-9\s\-_]/g, '').trim().substring(0, 100) || 'twitter';

  const job = {
    id: jobId, url, title: title || 'Twitter Video', safeTitle,
    format: format || 'video',
...
function startDownload(job) {
  ...
  ytdlp.on('close', code => {
    ...
    if (code === 0) {
      ...
      job.status = 'failed';
      const isPrivate = /login|private|permission|403|Forbidden|protected/i.test(stderr);
      job.error = isPrivate ? 'This video is private. Only public Twitter/X videos can be downloaded.' : 'Download failed. Please try again.';
    }
    processQueue();
  });
}
